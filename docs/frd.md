...
- `REF-07`: the commission (`REF-05`'s partner in the ladder: 2% of every Health Pass plan a referred member activates) is credited to the referrer's wallet automatically, exactly once per plan — a `REFERRAL_EARNINGS` ledger line and the balance move together, the referral becomes `PLAN_ACTIVATED`, and any level points that unlocks are paid. It is paid when the plan is approved by any path (`wallet_card_pay_referral` trigger), and also when a referral is recorded for a member who already holds approved plans (`referral_pay_earlier_plans`), so a plan bought before entering a referral ID still pays. The API repeats the step idempotently in `WalletService.approveCard` and `ReferralService.applySignupCode`. The apps show only money actually credited: "Sahakar money earned" and "Plans activated" are read from the referrer's own ledger, never projected. The root app adds each credit to its wallet balance and ledger once (`WalletService.applyReferralEarnings`); the agent app reads the server balance and ledger.
- `REF-06`: the Refer & Earn screen shows the referrer everyone who joined on their code, at the stage each has reached — Joined, Made a transaction, Plan activated — with the figures split into *Joined* (all sign-ups) and *Transacted* (what levels are cleared on), and a note on the current rung when sign-ups are waiting on a first order. The people list is folded to a one-line summary and opens on a tap (five at a time). The level bar and the points graph move with transactions from the first one: the graph's climb starts before Starter, so one of the two Starter asks for puts the marker halfway there. It re-reads on open, every 15 seconds while it is the visible page, on returning to the app, and on pull-to-refresh. Friends appear by first name and last initial only. Until the member's own code has loaded the code card says so and Invite is disabled — no placeholder code is ever shown or shared.
