...
A **Lab Admin** (`lab` role; create one from **Admins → Add**, role *Lab Admin*, or `pnpm seed:staff -- --login-id=lab_admin --name="Lab Admin" --password=… --role=LAB` in `backend/api`) sees the Dashboard, **Stores**, **Lab Orders** and **Lab Tests** — nothing else. `admin` and `superadmin` can open the same pages. Stores access has no role-based scoping of its own (one Lab Admin sees and edits every branch, the same as Lab Orders lists bookings from all of them), matching how staff mostly used to reach `shieldweb/src/pages/StoresPage.tsx` through `admin`/`superadmin` already.

**Which branches take lab bookings (migration `0057_lab_store_routing.sql`, applied).** `app.shield_store.offers_lab_collection` (default `true`) is a per-branch switch: a branch's detail view on the Stores page has a **"Turn off/on lab collection"** button next to Active/Inactive, and a badge ("Takes lab bookings" / "No lab bookings") beside the active/inactive one; **"+ New branch"** offers the same choice as a checkbox, on by default. Karinkallathani and Makkaraparamba were switched off by the migration itself, at the business's request; every other existing branch was left on. A booking's own branch is on `app.lab_booking.store_id` (nullable, `ON DELETE SET NULL`) — `LabBookingModal`'s detail view and the Lab Orders list (a **Branch** column and filter, `LEFT JOIN app.shield_store`) both show it, falling back to "—" for a booking placed before this migration or whose branch was never resolved.

In the apps, a member's lab checkout has its own **Branch** row (`LabCartScreen`, both apps): it defaults to the member's registered home branch when that branch still takes lab bookings, and is tappable to pick a different eligible one — root reuses `StorePickerSheet` (its existing registration picker) with a filter and different copy; `shield agent_invester/` reads live from `GET /v1/public/care/lab-stores` (`BookingService.listLabStores`, `care.controller.ts`) into its own small `LabStorePickerSheet`, rather than through its bundled, never-refreshed `StoreDirectory`, so a branch switched off here is reflected immediately rather than only after the next app release. `POST /v1/member/lab-bookings`'s `storeId` is optional and validated server-side against the same eligible list (`ForbiddenException` if the branch does not take lab bookings); omitting it falls back to the member's home branch when that is itself eligible, else leaves the booking's branch unset.
...
### Lab Tests → Test Master, Member packages, Categories

`LabsPage` (`/lab-tests`, open to the `lab`, `admin` and `superadmin` roles) has three tabs. **Test Master** (`components/labtests/LabTestMaster.tsx`) is the laboratory's own catalogue, laid out like its LIS "Test" screen: search a test by name, short name or Lis Code, edit it, then **Delete**, **New** or **Save**. **Member packages** is what members can actually book in the app, built from the Test Master's own tests. **Categories** is "Explore by health concern" — the icon grid both apps' Lab sections open with.
...
Data is in `app.lab_test`, `app.lab_test_group_item` and `app.lab_test_special_rate`. Apply, in order: `0046_lab_test_master.sql` (the tables), `0048_lab_test_rate_list_columns.sql` (`scheduled_days`, `reporting_time`, `lab_rate`), `0049_seed_lab_test_rate_list.sql` (the tests), then `0050_lab_test_source.sql` (the source tag; a no-op after a fresh 0049); all are also reflected in `app_schema.sql`. Saving is one SQL statement (`src/api/labTestSql.ts buildSaveStatement`) so a test and its rows save together or not at all. **Until 0046 and 0048 are applied the Test Master tab fails to load.**

**Categories and the package builder (migration `0055_lab_categories.sql`, applied to the live database).** `app.lab_category` — a name, an uploaded icon (`image`, a data URI, same shape as `app.product_category.image`), sort and active flag — is what "Explore by health concern" is built from, on both `LabTestMaster`'s test form (`category_id` on `app.lab_test`) and `app.lab_package` itself. A category can be deleted freely (`ON DELETE SET NULL`); the console still refuses it first while a test or package still carries it, asking the admin to move them off it deliberately. `care.controller.ts`'s `GET /v1/public/care/lab-categories` is what `shield agent_invester/` reads (root queries `app.lab_category` directly, like the rest of its Neon repositories) — each category carries a live count of its own active packages, never a stored figure.

**Member packages → "+ New package"** (and a package's own "Edit tests & details") is the first real create path `app.lab_package` has ever had: name, category, price/MRP, and a search-and-pick list from the Test Master — every active `TEST` **and `GROUP`**, never a `PACKAGE` (`components/labtests/LabPackageBuilderModal.tsx`). This is the same "package is built from a few named panels, each covering several markers" shape a real lab uses: pick "Liver Function Test" (a group the Test Master's own Set Grouptest tab already built from SGOT, SGPT, Bilirubin, …) and the whole panel goes on the package as **one** line, named after the group itself — not its four tests loose. A package typically ends up as 3–4 such groups (plus, if wanted, a loose extra test dropped straight in). The picker's All/Groups/Tests filter and each group's own badge (`Group · 4`) keep the two kinds apart while building.

**"Show in the app" — tests and group tests for members (migration `0056_lab_test_show_in_app.sql`, applied).** Until now only packages ever reached the apps. The Test Master form has a **Show in the app** switch (on by default for a test made there, off for the imported rate list; not offered for a Package-type test). A test or group test that is switched on **and Active** is listed to members as its own one-profile `app.lab_package` row (`source_test_id` set) — that is what a member actually books — which `syncLabTestListing` (`src/api/labTests.ts`) writes on every save: price = the test's Amount, MRP = its Rate, test count = the tests inside a group (1 for a single test), the test's category and reporting time. Switching it off, making it inactive or turning it into a Package hides the listing (kept, not deleted, so an earlier booking still points at something); deleting the test removes it, refused with a plain message if a member has booked it. These listings are what both apps show under **Top Profiles and Tests** (with "View all N tests ›"), each with a "+" to book; they are kept out of **Member packages** (which lists real packages only, `source_test_id IS NULL`) and out of "All Packages".

Saving writes both the real link (`app.lab_package_test_item`, so re-opening the package to change its selection reads back the actual tests/groups chosen, never guessed from `lab_profile`) and, derived from that same selection, `app.lab_profile` rows — one profile per chosen test or group, `parameters` set to the group's own real test count (1 for a loose test). `app.lab_package.profile_count` is how many lines that is (one per chosen item); `test_count` is the true sum of individual tests across everything chosen — a package of "LFT (4) + KFT (3) + HbA1c" is `profile_count = 3`, `test_count = 8`. The package card both apps already render (`LabProfile.parameters`, "CBC · 24 parameters") needs no changes at all to show a console-built package correctly. Editing a package's price/category/other fields alone never touches its test selection or these two counts — only actually changing which tests/groups are chosen does.
