...
**Categories and the package builder (migration `0055_lab_categories.sql`, written and verified but NOT yet applied to the live database).** `app.lab_category` — a name, an uploaded icon (`image`, a data URI, same shape as `app.product_category.image`), sort and active flag — is what "Explore by health concern" is built from, on both `LabTestMaster`'s test form (`category_id` on `app.lab_test`) and `app.lab_package` itself. **Member packages → "+ New package"** (and a package's own "Edit tests & details") is the first real create path `app.lab_package` has ever had: name, category, price/MRP, and a search-and-pick list of the Test Master's own active single tests (`components/labtests/LabPackageBuilderModal.tsx`). Saving writes both the real link (`app.lab_package_test_item`, so re-opening the package to change its tests reads back the actual selection) and, derived from the same tests, `app.lab_profile` rows — the package card both apps already render needs no changes at all to show a console-built package. A category can be deleted freely (`ON DELETE SET NULL`); the console still refuses it first while a test or package still carries it, asking the admin to move them off it deliberately. `care.controller.ts`'s `GET /v1/public/care/lab-categories` is what `shield agent_invester/` reads (root queries `app.lab_category` directly, like the rest of its Neon repositories) — each category carries a live count of its own active packages, never a stored figure.
