...
## Lab categories ("Explore by health concern")

`app.lab_category` (migration `0055_lab_categories.sql`) is a name, an uploaded icon and a sort order, managed on the console's Lab Tests → Categories tab. Both apps' Lab landing screen (`lib/module/labtest/lab_test_screen.dart`, byte-identical between the two) show it as an icon grid right under the search field, each tile carrying a live count of its own active packages; tapping one pushes `TopPackagesScreen(category: …)`, the same package list "See all" opens, filtered to that category. Root's `CareRepository.fetchLabCategories` (`lib/data/neon/care_repository.dart`) queries `app.lab_category` directly; `shield agent_invester/`'s reads `GET /v1/public/care/lab-categories` (`care.controller.ts` → `CareService.listLabCategories`, cached the same 300s as `listLabPackages`). A category is entirely optional on both `app.lab_test` (set from the Test Master's own create/edit form) and `app.lab_package` (set by the package builder, see [admin-console.md](admin-console.md#lab-tests--test-master-member-packages-categories)) — `ON DELETE SET NULL` either way, so deleting a category never blocks and never deletes what it labelled.

### Top Profiles and Tests

The Lab landing screen lists single tests and group tests beside the package cards, in the reference layout: Top Packages, Book via Call/WhatsApp, **Top Profiles and Tests** (five rows — icon, name, price, MRP struck through, "% OFF", "· N tests", and a "+" that opens the patient-count sheet — then "View all N tests ›" to `TopProfilesScreen`), then Explore by health concern. They come down the same read as the packages: a test switched on with "Show in the app" is a one-profile `app.lab_package` row (`source_test_id` set, migration `0056`), which is what `lab_booking` needs to point at, so booking one is the ordinary package booking. `LabPackage.isProfile` (root: `source_test_id IS NOT NULL`; agent app: `sourceTestId != null` in `GET /v1/public/care/lab-packages`) tells the two apart — the Top Packages strip and "All Packages" show real packages only, a category page shows both. `profile_tile.dart` / `top_profiles_screen.dart` are byte-identical in both apps.

### The lab checkout's Branch row (migration `0057_lab_store_routing.sql`)

Booking a lab test routes to a branch the same way a standard order already does (`storeCode` at checkout), a concept `app.lab_booking` had none of before this migration. `LabCartScreen`'s "Branch" row (`_BranchRow`, both apps) shows the branch a booking will collect from and opens a picker to change it, restricted to branches with `shield_store.offers_lab_collection = true` (a per-branch admin toggle, see [admin-console.md](admin-console.md#lab-orders--the-lab-admins-desk)):

- **Root** reuses `StorePickerSheet` — the same sheet `registration_screen.dart` already opens to choose a home branch — now with an optional `filter` (`(s) => s.offersLabCollection`) and a `title`/`subtitle` override, rather than a second bespoke widget. `StoreDirectory.all` is root's live-synced branch list (`StoreCatalog` pulls `app.shield_store` on launch), so the filter reads current data.
- **`shield agent_invester/`** cannot do the same: its own `StoreDirectory`/`ShieldStore` (`lib/module/registration/shield_store.dart`) is a static, bundled seed that is never refreshed from the backend (`StoreRepository` there only ever fetches a phone-lookup map, not the full branch list) — reusing it would show stale or wrong lab eligibility. Instead it has its own small model, `LabStore` (`lib/module/labtest/lab_store.dart`), fetched live every time the picker opens via `CareRepository.fetchLabStores()` → `GET /v1/public/care/lab-stores`, into a dedicated `LabStorePickerSheet`.

Both default the row to the member's own registered home branch when that branch is itself lab-eligible, and fall back to "Choose a branch" otherwise. `OrderRepository.saveLabBookings` (root: direct Neon insert; agent app: `POST /v1/member/lab-bookings`) threads the chosen branch through as `storeCode`/`storeId`; the backend (`BookingService.bookLabTest`) validates a client-sent `storeId` against the same eligible list and, when none is sent, falls back to the member's `home_store_id` server-side too — so a booking always resolves the same way regardless of which path wrote it.
