...
Identifiers still say `shield` on purpose, because renaming them would break builds, sign-in, deployments and existing data: the `shield` Dart package and `shield_*` file names, `shieldweb/`, `shield agent_invester/`, the `app.shield_store` table, the Firebase project `shield-zabnix`, Android/bundle ids, `SHD-` branch, agent and investor codes and order numbers, and promo codes such as `SHIELD20`. The **Member ID** (each member's own invite code) is the exception: it is issued as `SAHAKAR-####`, existing members were moved over by migration `0051_member_id_sahakar_prefix.sql`, and a code shared earlier as `SHIELD-####` is still accepted at registration. Two things are left unchanged pending a decision: the plan tier names ("Silver / Gold / Platinum Shield", stored in `app.membership_tier`) and the branch bank **account holder names** on the payment screen (`shield_payee.dart`), which must match the real bank accounts. The logo image `assets/logos/shield_wordmark.png` still reads "Mission S.H.I.E.L.D" and needs new artwork.
