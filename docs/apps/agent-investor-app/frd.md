...
| PAR-AGT-03 | Show a recruit awaiting approval as a locked position in the team tree — name, reference code, and a lock icon in place of opening their detail screen — until an admin approves or rejects; the chevron below them still fans out the tier they would head (real reports if any exist, open positions otherwise), since a recruiter needs to see their team's whole shape whether or not each seat is confirmed yet | `agent_team_tree_screen.dart` (`_MindPill.locked` gates the body tap/detail screen only; `canExpand` is independent of `agent.isApproved`), `AgentApprovalStatus.pending` |
