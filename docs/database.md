...
- `region → state → district → assembly → lsgd → ward` — one table per administrative tier (UUIDv7 keys), replacing the earlier `agent_geo_node` slot table. No seed rows ship with the migration; the real Kerala data is loaded separately by `dart run backend/db/seed_kerala_geo.dart` from the Suvida LSG source spreadsheet (not committed). This is the full chain the source data covers — Kerala's rural local government also has block panchayat (152) and district panchayat (14) tiers above grama panchayat, but the Suvida LSG workbook has no rows for either, so they are not modeled here; treat them as a known gap, not an oversight, until a source for them is available.
- `lsgd_code_key` / `ward_code_key` (migration `0058_lsgd_code_integrity.sql`, applied to the live database) — partial unique indexes on `lsgd.code` / `ward.code` (excluding the `''` default), since the source's own government-assigned codes are globally unique but nothing enforced that before. The same migration backfills `lsgd.sort`, which the original seed script left at the column default (0) for every row; the script now derives it from the code's own trailing digits.
...
- `lab_category` (migration `0055_lab_categories.sql`, applied to the live database) — "Explore by health concern": a name, an uploaded `image` (a data URI, same shape as `product_category.image`), sort and active flag. `lab_test.category_id` and `lab_package.category_id` both reference it, `ON DELETE SET NULL` on either. `lab_package_test_item` (package_id, test_id, sort; test `ON DELETE RESTRICT`) is the same migration's other addition — the console's package builder (Lab Tests → Member packages → "+ New package") writes it alongside `lab_package`/`lab_profile`, keeping a real link back to whichever `lab_test` rows (single tests **or group tests**) a package was assembled from, not just the free-text `lab_profile` rows the app renders. A chosen group test becomes one `lab_profile` row named after the group, `parameters` set to its own real test count — a package built from a few groups (a common "3–4 panels" shape) still gets an honest `test_count` (the sum across every group) alongside `profile_count` (one row per chosen group/test).
- `shield_store.offers_lab_collection` and `lab_booking.store_id` (migration `0057_lab_store_routing.sql`, applied to the live database) — which branches a lab booking may route to, and which one a given booking actually went to. `offers_lab_collection` (`boolean NOT NULL DEFAULT true`) is the Stores page's per-branch "Turn off/on lab collection" switch; `store_id` (`bigint REFERENCES shield_store(id) ON DELETE SET NULL`, indexed) is nullable so a booking placed before this migration, or one whose branch never resolved, reads as "no branch on record" rather than failing. `BookingService.listLabStores` (`GET /v1/public/care/lab-stores`) is every `is_active AND offers_lab_collection` branch; `bookLabTest` resolves a booking's `store_id` from a client-sent `storeId` (validated against that same list) or, when omitted, the member's own `home_store_id` when that branch is itself eligible — mirroring how a standard order already resolves its `store_id`, the pattern this migration was built to match. The migration itself also backfilled every existing branch to `is_active = true` and switched `offers_lab_collection` off for Karinkallathani (`SHD-KKT`) and Makkaraparamba (`SHD-MKP`) specifically, at the business's request.
