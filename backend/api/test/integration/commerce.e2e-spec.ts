...
describe('Commerce (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await db.insert(adminUser).values({
      ...
      name: 'Store A Staff',
      firebaseUid: 'staff-store-a',
      role: 'PHARMACY',
      storeId: storeA.id,
    });
    firebase.register('store-a-token', { uid: 'staff-store-a', email: 'storea@example.com' });

    await db.insert(adminUser).values({
      email: 'storeb@example.com',
      name: 'Store B Staff',
      firebaseUid: 'staff-store-b',
      role: 'PHARMACY',
      storeId: storeB.id,
    });
    firebase.register('store-b-token', { uid: 'staff-store-b', email: 'storeb@example.com' });

    memberAccessToken = (
      await request(app.getHttpServer()).post('/v1/member/auth/session').send({ idToken: 'member-token' }).expect(200)
    ).body.accessToken;
    storeAStaffToken = (
      await request(app.getHttpServer()).post('/v1/staff/auth/session').send({ idToken: 'store-a-token' }).expect(200)
    ).body.accessToken;
    storeBStaffToken = (
      await request(app.getHttpServer()).post('/v1/staff/auth/session').send({ idToken: 'store-b-token' }).expect(200)
    ).body.accessToken;
