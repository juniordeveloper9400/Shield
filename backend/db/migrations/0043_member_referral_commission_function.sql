-- ============================================================================
--  0043 · Member-to-member referral commission — approval-time crediting
-- ============================================================================
--  0042 added the schema (the REFERRAL_EARNINGS wallet_entry kind, the
--  referral_level seed rows, users.referral_level_awarded). This migration
--  is what actually pays out on it, inside `app.approve_wallet_card_activation`
--  — the same Postgres function shieldweb's admin console calls on every
--  real wallet-card approval (see `shieldweb/src/api/activations.ts`),
--  mirroring `WalletService.approveCard`'s TypeScript version of this same
--  logic for backend/api's own staff route.
--
--  What's added, appended after the existing agent-commission split and
--  independent of it (a referral and a direct sale are different
--  relationships to the same card — both are owed on the same activation):
--
--   - If the card owner has a `referred_by_member_id`, credit that referrer
--     2% of the load as a REFERRAL_EARNINGS wallet_entry, every time this
--     member activates a plan (not just the first).
--   - Advance (or, if none exists yet, create) the `app.referral` edge for
--     that pair to PLAN_ACTIVATED, recording plan_amount and accumulating
--     commission_amount — the real total `getProgress`'s `sahakarMoneyEarned`
--     now reads back, replacing what used to be a client-side 2% projection.
--   - Re-check the referrer's own referral_level standing
--     (app.award_referral_level_points, new function this migration adds)
--     and pay real reward points the moment their direct-referral count
--     crosses a rung — a plan activation is one of the two events that can
--     move that count (a referred member's first paid order is the other,
--     handled in `order.service.ts`'s checkout).
--
--  Idempotent — safe to run more than once:
--    dart run backend/db/apply_migration.dart backend/db/migrations/0043_member_referral_commission_function.sql --yes
-- ============================================================================
