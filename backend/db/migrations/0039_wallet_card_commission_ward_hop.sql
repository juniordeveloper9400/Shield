-- ============================================================================
--  0039 · Health Pass commission split — extend the chain walk to 6 hops
-- ============================================================================
--  Same rule as 0036-0038, one hop further — and the last one, since WARD is
--  the deepest level in app.agent_level (NATIONAL, REGION, STATE, DISTRICT,
--  ASSEMBLY, LSGD, WARD). The ward-level numbers just given (ward 60% /
--  lsgd 10% / assembly 6% / district 5% / state 4% / region 3% / national
--  2% / reserved 10%) are that exact same decaying chain-walk one hop
--  deeper still: a WARD seller's hop 1 is a real lsgd agent, hop 2 a real
--  assembly agent, hop 3 a real district agent, hop 4 a real state agent,
--  hop 5 a real region agent, hop 6 the national agent ->
--  600/100/60/50/40/30/20/100 on a 10,000 rupee plan, i.e. 10% reserved —
--  matching exactly what was specified. This migration only extends
--  v_hop_rates with that sixth entry; the walk logic itself (0036) is
--  unchanged.
--
--  Every level now has a specified rate — v_hop_rates covers the deepest
--  possible seller (WARD) in full, with nothing left to extend unless a new
--  level is ever added below WARD.
--
--  Idempotent — safe to run more than once:
--    dart run backend/db/apply_migration.dart backend/db/migrations/0039_wallet_card_commission_ward_hop.sql --yes
-- ============================================================================
