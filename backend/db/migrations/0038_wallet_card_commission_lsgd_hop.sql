-- ============================================================================
--  0038 · Health Pass commission split — extend the chain walk to 5 hops
-- ============================================================================
--  Same rule as 0036/0037, one hop further. The lsgd-level numbers just
--  given (lsgd 60% / assembly 10% / district 6% / state 5% / region 4% /
--  national 3% / reserved 12%) are that exact same decaying chain-walk one
--  hop deeper: an LSGD seller's hop 1 is a real assembly agent, hop 2 a
--  real district agent, hop 3 a real state agent, hop 4 a real region
--  agent, hop 5 the national agent -> 600/100/60/50/40/30/120 on a 10,000
--  rupee plan, i.e. 12% reserved — matching exactly what was specified.
--  This migration only extends v_hop_rates with that fifth entry; the walk
--  logic itself (0036) is unchanged.
--
--  Only ward is left unspecified — a seller 6 hops below national (a ward
--  agent, if their own real chain runs all the way up) pays out at most
--  these 5 known hops and reserves the rest. Extend v_hop_rates again once
--  that number is given.
--
--  Idempotent — safe to run more than once:
--    dart run backend/db/apply_migration.dart backend/db/migrations/0038_wallet_card_commission_lsgd_hop.sql --yes
-- ============================================================================
...
DECLARE
    ...
    v_hop_rates        numeric[] := ARRAY[0.10, 0.06, 0.05, 0.04, 0.03];
    v_hop              int;
