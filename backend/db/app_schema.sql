...
-- Uploaded customer-review video bytes. Incomplete chunked uploads are kept
-- separate from the catalogue row and cannot be served until verified.
...
CREATE INDEX customer_review_video_media_incomplete_idx
  ON app.customer_review_video_media(created_at) WHERE upload_complete = false;
...
CREATE TABLE app.prescription_medicine (
    ...
    product_id       bigint REFERENCES app.product(id) ON DELETE SET NULL,
    status           app.prescription_medicine_status NOT NULL DEFAULT 'AVAILABLE' -- migration 0024: pharmacist-only stock note, never shown to the member
);
...
-- The agent-registration approval queue (migration
-- shield agent_invester/backend/db/migrations/0011_agent_request.sql). A recruit
-- submitted from the app lands here as PENDING; the admin console ("Agent
-- approvals") approves it into an app.agent row at a confirmed level/parent/area,
-- or rejects it with a reason. app.agent therefore only ever holds real,
-- approved agents.
